...
def build_correction_dataframe(
    ...
    mask_needs_corr = df["match_status"].eq("match_needs_correction")
    mask_has_suggestion = df["suggested_tax_code_1"].notna()
    ...
    if "_merge" in df.columns:
        ...
        mask_in_range = pd.Series(True, index=df.index)
    
    if "date_within_tolerance" in df.columns:
        mask_in_range &= df["date_within_tolerance"].fillna(False)
    ...
    if "action" in df.columns and allowed_actions is not None:
        ...
        mask_action = df["action"].isin(allowed_actions)
    else:
        mask_action = pd.Series(True, index=df.index)

    corr_mask = mask_needs_corr & mask_has_suggestion & mask_in_range & mask_action
    df_corr = df[corr_mask].copy()
    ...
    out = df_corr.rename(columns=rename_map)

    # 7) Keep only the columns we wan, in the desired order
    out = out[out_cols]
